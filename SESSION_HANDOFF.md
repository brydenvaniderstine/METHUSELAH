...
1. **Verify source switching live on methuselah.ca** — after this session's deploy, confirm RHR tile shows "● OURA LIVE" while the Oura token is valid, and manually confirm (e.g. by temporarily clearing `oura_token` from localStorage, or after the token expires 2026-07-13) that it switches to "● GEN3 BLE" automatically. Logic was verified directly against `engine/` (not through a browser — the dev server couldn't be started in this environment) so a real browser check is still owed.
2. **Decide whether SpO2 becomes a fifth command vector** — it's now resolved in `engine/sources.js` (telemetry only) since condition #3 closed, but adding `THRESHOLDS.spo2`/`COMMANDS.spo2` and a cascade branch in `engine/index.js` is a deliberate v3 discussion, not a default next step. Needs an owner decision on the threshold value and priority position before implementing.
3. **Track B — remaining conditions before completion**: #1 sleep_state full-night transitions (not started), #2 HRV redefined — 1/3 evening activity pulls with 0x5D confirmed, #4 0x77/0x7E/0x7F decoders still IN PROGRESS, #5 comparison CSV needs 14 consecutive nightly rows with both Gen3+Gen4 (currently at 9, several Gen4-only). Condition #3 (SpO2) is now CLOSED — see last session summary.
...
7. **Rotate the GitHub PAT embedded in `origin` remote URL** — found in plaintext via `git remote -v` last session. Recommend switching to SSH or a credential helper.
